    ...
    try:
        while True:
            ...
            if ":" in data:
                ...
                if (
                    campaign_id in manager.chat_connections
                    ...
                    await websocket.send_text(
                        f"[{timestamp}] Система: Користувач {target_id} не в мережі."
                    )
            else:
                await websocket.send_text(
                    f"[{timestamp}] Система: Неправильний формат. Використовуйте 'id_одержувача: повідомлення'"
                )
