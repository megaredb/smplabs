    ...
    loginMutation.mutate(
      ...
      },
      {
        ...
        onError: (err) => {
          ...
          errorMessage =
            (err as any)?.response?.data?.detail || $_('auth.errorDefault'); 
        }
      }
    );
  }
  ...
  <Card class="w-full max-w-md border-slate-200/60 bg-white/50 shadow-lg backdrop-blur-xl">
    ...
    <CardFooter class="flex justify-center">
      <p class="text-sm text-slate-500">
        {$_('auth.noAccount')}
        <a href={resolve('/register')} class="font-medium text-blue-600 transition-colors hover:text-blue-700 hover:underline">
          {$_('auth.registerLink')}
        </a>
      </p>
    </CardFooter>
