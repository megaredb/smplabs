	...
	{:else if statsQuery.isError}
		<Card class="border-red-200 bg-red-50/50">
			<CardContent class="py-10 text-center text-red-600">
				...
				<Button variant="outline" class="mt-4" onclick={fetchStats}
					>{$_('visitStat.retryBtn')}</Button
				>
			</CardContent>
		</Card>
	{:else if stats}
			...
			>
				<CardHeader class="flex-none border-b border-slate-100 bg-slate-50/50 pb-4">
					<div class="flex items-center gap-2">
						...
						<CardTitle class="text-lg font-semibold text-slate-700"
							>{$_('visitStat.totalVisitsTitle')}</CardTitle
						>
					</div>
				</CardHeader>
				<CardContent class="flex grow flex-col items-center justify-center py-12">
					...
					<div class="mt-4 rounded-full bg-slate-100 px-3 py-1 text-sm font-medium text-slate-500">
						{$_('visitStat.pageLabel')}
						<span class="font-mono text-blue-600">{stats.page_url}</span>
					</div>
				</CardContent>
			</Card>
			...
			>
				<CardHeader class="flex-none border-b border-slate-100 bg-slate-50/50 pb-4">
					<div class="flex items-center gap-2">
						...
						<CardTitle class="text-lg font-semibold text-slate-700"
							>{$_('visitStat.userVisitsTitle')}</CardTitle
						>
					</div>
				</CardHeader>
				<CardContent class="flex grow flex-col items-center justify-center py-12">
