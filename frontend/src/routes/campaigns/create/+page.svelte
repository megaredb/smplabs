<script lang="ts">
	...
	function handleSubmit(event: Event) {
		...
		if (!currentUserId) {
			errorMessage = 'Ви не авторизовані! Увійдіть в акаунт.';
			return;
		}
		...
		createMutation.mutate(
			...
			},
			{
				...
				onError: (error: unknown) => {
					...
					errorMessage = err.response?.data?.detail || 'Не вдалося створити збір. Перевірте дані.';
				}
			}
		);
	}
...
<div class="mx-auto mt-6 max-w-2xl rounded-xl border border-gray-200 bg-white p-8 shadow-sm">
	<div class="mb-8">
		...
		<p class="mt-1 text-gray-500">Заповніть деталі, щоб розпочати залучення коштів.</p>
	</div>
	...
	{#if isSuccess}
		<div
			class="mb-6 flex animate-bounce items-center rounded-lg border border-green-200 bg-green-50 p-4 text-green-700"
		>
			<svg
	...
	<form onsubmit={handleSubmit} class="space-y-6">
		<div>
			<label for="title" class="mb-1 block text-sm font-medium text-gray-700">Назва збору</label>
			<input
				id="title"
				type="text"
				bind:value={title}
		...
		<div class="flex justify-end space-x-4 border-t border-gray-100 pt-4">
			<a
				...
				class="rounded-lg px-5 py-2 font-medium text-gray-700 transition-colors hover:bg-gray-100"
			>
				Скасувати
			</a>
			<button
