...
<div class="mx-auto mt-6 max-w-2xl rounded-xl border border-gray-200 bg-white p-8 shadow-sm">
	...
	{#if isSuccess}
		<div
			class="mb-6 flex items-center rounded-lg border border-green-200 bg-green-50 p-4 text-green-700"
		>
	...
	<form onsubmit={handleSubmit} class="space-y-6">
		<div>
			<label for="title" class="mb-1 block text-sm font-medium text-gray-700"
				>{$_('createCampaign.titleLabel')}</label
			>
			<input
				id="title"
				type="text"
		...
		<div>
			<label for="description" class="mb-1 block text-sm font-medium text-gray-700"
				>{$_('createCampaign.descLabel')}</label
			>
			<textarea
				id="description"
		...
		<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
			<div>
				<label for="amount" class="mb-1 block text-sm font-medium text-gray-700"
					>{$_('createCampaign.targetLabel')}</label
				>
				<input
					id="amount"
					type="number"
		...
		<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
			<div>
				<label for="endDate" class="mb-1 block text-sm font-medium text-gray-700"
					>{$_('createCampaign.endDateLabel')}</label
				>
				<input
					id="endDate"
					type="date"
			...
			<div>
				<label for="imageUrl" class="mb-1 block text-sm font-medium text-gray-700"
					>{$_('createCampaign.imageUrlLabel')}</label
				>
				<input
					id="imageUrl"
					type="url"
					bind:value={imageUrl}
					placeholder={$_('createCampaign.imageUrlPlaceholder')}
					class="w-full rounded-lg border border-gray-300 px-4 py-2 transition-all outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500"
				/>
			</div>
		</div>
