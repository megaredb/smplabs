<script lang="ts">
	...
	function openEditDialog() {
		...
		if (campaign) {
			...
			editEndDate = (campaign as any).end_date
				? new Date((campaign as any).end_date).toISOString().split('T')[0]
				: '';
...
<div class="mx-auto max-w-4xl space-y-6">
	...
	{:else}
		...
		{#if campaign}
			...
			{@const isClosed = (campaign as any).end_date
				? new Date((campaign as any).end_date) < new Date()
				: false}
						...
						{:else}
							...
							>
								<span class="text-lg font-medium text-slate-400"
									>{$_('campaignDetails.noImage')}</span
								>
							</div>
						{/if}
						<CardHeader>
							...
							<div
								class="mt-4 flex items-center gap-2 border-t border-slate-100 pt-4 text-slate-600"
							>
								<div
									class="relative flex h-8 w-8 items-center justify-center rounded-full bg-slate-100"
								>
									<User class="h-4 w-4 text-slate-500" />
									{#if (campaign as any).is_verified}
										<div class="absolute -right-1 -bottom-1 rounded-full bg-white">
								...
								<span class="flex items-center gap-2 text-sm">
									...
									{#if (campaign as any).is_verified}
										<span
											...
											>{$_('campaignDetails.verified')}</span
										>
									{/if}
								</span>
							</div>
								...
								<div class="space-y-6">
									<div class="flex items-center justify-between">
										<h3 class="text-lg font-semibold text-slate-900">
											{$_('campaignDetails.reportsTitle')}
										</h3>
										{#if currentUserId === campaign.organizer_id}
											<Button
												variant="outline"
										...
										<div class="mt-4 space-y-6">
											{#each reportsQuery.data as any as report}
												<Card class="overflow-hidden border border-slate-200 shadow-sm">
													...
													<CardContent class="p-5 pt-2 whitespace-pre-wrap text-slate-700">
														{report.description}
													</CardContent>
												</Card>
											{/each}
									...
									{:else}
										<div
											class="rounded-lg border border-dashed border-slate-300 bg-slate-50 py-10 text-center text-slate-500"
										>
						...
						<CardContent class="space-y-6 pt-6">
							<div class="space-y-2">
								<div class="flex items-end justify-between text-sm">
									...
									<span class="mb-1 text-slate-500"
										>{$_('campaignDetails.of')} {targetAmount} ₴</span
									>
								</div>
							...
							<Button
								size="lg"
								class="w-full py-6 text-base font-semibold shadow-md transition-all active:scale-[0.98] {isClosed ||
								remainingAmount <= 0
					...
					<form onsubmit={handleEdit} class="space-y-4 py-4">
						...
						<div class="space-y-2">
							...
							<select
								...
								class="flex h-10 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none"
							>
						...
						<Dialog.Footer>
							...
							<Button type="submit" disabled={updateMutation.isPending}
								>{$_('campaignDetails.saveBtn')}</Button
							>
						</Dialog.Footer>
					</form>
				</Dialog.Content>
					...
					<form onsubmit={handleCreateReport} class="space-y-4 py-4">
						...
						<div class="space-y-2">
							...
							<textarea
								...
								required
								rows="5"
								class="w-full resize-y rounded-md border border-slate-300 bg-transparent px-3 py-2 text-sm placeholder:text-slate-400 focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:outline-none"
							></textarea>
						</div>
						<div class="space-y-2">
						...
						<Dialog.Footer class="pt-4">
							<Button type="button" variant="outline" onclick={() => (isReportDialogOpen = false)}
								>{$_('campaignDetails.cancelBtn')}</Button
							>
							<Button
								type="submit"
			...
			<form onsubmit={handleComplaint} class="space-y-4 py-4">
				<div class="space-y-2">
					...
					<textarea
						...
						required
						rows="4"
						class="w-full resize-y rounded-md border border-slate-300 px-3 py-2 text-sm focus:ring-2 focus:ring-red-500 focus:outline-none"
					></textarea>
				</div>
				<Dialog.Footer>
					<Button type="button" variant="outline" onclick={() => (isReportCampaignOpen = false)}
						>{$_('campaignDetails.cancelBtn')}</Button
					>
					<Button
						type="submit"
