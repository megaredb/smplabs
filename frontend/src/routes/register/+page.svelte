    ...
    <Card class="w-full max-w-md border-slate-200/60 bg-white/50 shadow-lg backdrop-blur-xl">
        ...
        <CardContent>
            ...
            <form onsubmit={handleRegister} class="space-y-4">
                <div class="space-y-2">
                    ...
                    <Input id="name" type="text" bind:value={name} required placeholder={$_('auth.namePlaceholder')} />
                </div>
        ...
        <CardFooter class="flex justify-center">
            <p class="text-sm text-slate-500">
                ...
                <a href={resolve('/login')} class="font-medium text-blue-600 transition-colors hover:text-blue-700 hover:underline">
                    {$_('auth.loginLink')}
                </a>
            </p>
        </CardFooter>
