<script lang="ts">
	...
	let activeCampaigns = $derived(
		campaigns.filter((campaign) => (campaign.current_amount ?? 0) < Number(campaign.target_amount))
			.length
	);
	...
	function progressValue(campaign: CampaignResponse) {
		return Math.min(
			100,
			Math.round(((campaign.current_amount ?? 0) / Number(campaign.target_amount)) * 100)
		);
	}
...
<div class="mx-auto max-w-7xl space-y-6">
	<div
		class="flex flex-col items-center justify-between rounded-3xl border border-slate-200 bg-white/90 p-6 shadow-sm sm:flex-row"
	>
		<div>
		...
		<div class="mt-4 sm:mt-0">
			...
			{:else}
				<Button
					...
					class="flex items-center gap-2 rounded-full bg-black px-6 text-white hover:bg-slate-800"
				>
	...
	<section class="rounded-3xl border border-slate-200 bg-white/90 p-6 shadow-sm">
		<div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
			...
			<div class="flex flex-wrap gap-3">
				<Button variant="outline" href={resolve('/campaigns')}
					>{$_('userStats.allCampaignsBtn')}</Button
				>
		...
		<Card class="border-slate-200 shadow-sm">
			...
			<CardContent>
				...
				<CardDescription class="mt-2 text-slate-500"
					>{$_('userStats.statCampaignsDesc')}</CardDescription
				>
			</CardContent>
		</Card>
		<Card class="border-slate-200 shadow-sm">
			...
			<CardContent>
				...
				<CardDescription class="mt-2 text-slate-500"
					>{$_('userStats.statActiveDesc')}</CardDescription
				>
			</CardContent>
		</Card>
		<Card class="border-slate-200 shadow-sm">
			...
			<CardContent>
				...
				<CardDescription class="mt-2 text-slate-500"
					>{$_('userStats.statRaisedDesc')}</CardDescription
				>
			</CardContent>
		</Card>
		<Card class="border-slate-200 shadow-sm">
			...
			<div class="flex w-full max-w-md items-center gap-3">
				<div class="relative w-full">
					...
					<Input
						...
						placeholder={$_('userStats.searchCampaignPlaceholder')}
						class="pl-10"
					/>
				</div>
			</div>
